...
//   // Free auto-generation functions (no API calls)
//   function generateTagsFromTitle(title, platform) {
//     const commonMusicTags = [
//       'chill', 'ambient', 'electronic', 'rock', 'pop', 'hip-hop', 'jazz', 'classical',
//       'indie', 'alternative', 'dance', 'house', 'techno', 'reggae', 'country',
//       'folk', 'blues', 'metal', 'punk', 'r&b', 'soul', 'funk', 'disco',
//       'latin', 'world', 'experimental', 'acoustic', 'upbeat', 'mellow',
//       'energetic', 'relaxing', 'nostalgic', 'modern', 'vintage', 'underground'
//     ];

//     const titleWords = title.toLowerCase().split(/\s+/);
//     const platformTag = platform.toLowerCase();
    
//     // Find matching tags from title
//     const matchedTags = commonMusicTags.filter(tag => 
//       titleWords.some(word => word.includes(tag) || tag.includes(word))
//     );

//     // Add platform as a tag
//     const finalTags = [platformTag, ...matchedTags];
    
//     // Add some random relevant tags if not enough matches
//     if (finalTags.length < 3) {
//       const remainingTags = commonMusicTags.filter(tag => !finalTags.includes(tag));
//       const randomTags = remainingTags.sort(() => 0.5 - Math.random()).slice(0, 3 - finalTags.length);
//       finalTags.push(...randomTags);
//     }

//     return finalTags.slice(0, 5); // Return max 5 tags
//   }

//   function generateDescriptionFromTitle(title, platform) {
//     const descriptions = [
//       `A curated collection of tracks from ${platform} featuring ${title}.`,
//       `Discover amazing music in this ${title} playlist on ${platform}.`,
//       `Handpicked songs for ${title} lovers, available on ${platform}.`,
//       `The ultimate ${title} experience, carefully selected for ${platform}.`,
//       `Immerse yourself in ${title} with this specially crafted ${platform} playlist.`
//     ];
